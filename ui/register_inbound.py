...
class RegisterInbound(BaseFrame):
    def __init__(self, parent, controller, db):
        ...
        title = ctk.CTkLabel(self, text="Register inbound", font=("Arial", 24))
        title.pack(padx=20, pady=20)

        frame = ctk.CTkFrame(self)
        frame.pack(padx=20, pady=20)
        label1 = ctk.CTkLabel(frame, text="Slot coordinates")
        label1.pack()
        ...
        self.use_full_pallet = ctk.CTkCheckBox(grid, text="Use full pallet?")
        self.use_full_pallet.grid(row=2, column=0, columnspan=2, padx=10, pady=10)

        register_button = ctk.CTkButton(
            self, text="Register", command=self.register_inbound
        )
    ...
    def register_inbound(self) -> None:
        ...
        if article_code == "":
            raise Exception("Article code cannot be empty.")

        try:
            quantity = int(self.quantity.get())
        except ValueError:
            tk.messagebox.showerror("RackMaster", "Quantity must be an integer.")
            return
        ...
        self.reset()
        tk.messagebox.showinfo("RackMaster", "Inbound registered successfully.")
